...
- The candidate products are real items/materials drawn from the contractor's own reference pages — treat them as the contractor's design language and available materials/style, not a strict SKU catalog the visible item must identically match. If a visible item is reasonably similar to a candidate product — same general category and a comparable material, finish, or style — set catalogProductId to that candidate's "id", even if it isn't an identical product. This keeps the quote grounded in what the contractor actually offers instead of leaving everything unpriced. Prefer the closest reasonable candidate over null; only use null when nothing in the candidate list is plausibly related to the item at all (e.g. a small decorative object with no comparable item among the candidates).
- Separately from that matching judgment, be precise about the id string itself: COPY it EXACTLY, character for character, from the candidate list — never shorten, reformat, or guess at it. If you're unsure of the exact id string for an item you do want to match, use null rather than guessing at the string.
- Do not invent prices. Pricing is computed separately from the catalog using whatever catalogProductId you provide — matching an item to a candidate never means inventing that candidate's price if the candidate itself has none.
